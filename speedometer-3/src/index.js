...
document.addEventListener('DOMContentLoaded', function () {
    ...
    //     $('#speed-range-title').text('Speed Level: ' + $(this).val());
    // });
    // use on input to update value in real time
...
ipcRenderer.on('on-serial-open', () => {
    ...
    // });
    // get message 1 from input element of id 'message-1' use jquery
...
function onIncommingData(data) {
    ...
    var speed = localStorage.getItem('speed');
    if (speed != null) {
        d = parseInt(speed); // 0- 100
        if (isNaN(d)) {
            d = 0;
        }
        // map speed from 0-100 to 0-10
        d = d / 100 * 50;
        if (d < 0) {
            d = 0;
        }
